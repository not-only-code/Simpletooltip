...
(function($) {
	...
	function getTooltipTag(_event) {
		...
		if (title_.length > 0) return('<div id="simple-tooltip-'+_event.data.index+'" class="simple-tooltip '+getTooltipPosition($(_event.target))+' '+getThemeName($(_event.target))+'">'+title_+'<span class="arrow">&nbsp;</span></div>');
		return false;
	}
	//--------- --
	//
	...
	function mouseOut(_event) {
		var exist_ = $('body').find('#simple-tooltip-'+_event.data.index);
		if(exist_.length == 0) {
			_debug('no existe el elemento: cancela');
			_event.preventDefault();
			return false;
		}
		if(exist_.css('opacity') == 0) {
			_debug('opacidad 0: eliminarrrrrr');
			_event.preventDefault();
			exist_.remove();
			return false;
		}
	...
	//
	function addActions() {
		$('.simpletooltip').each(function(index, value) {
	...
	//
	function setSettings(settings_) { settings = $.extend(defaults, settings_); };
	//
	//
	$.simpletooltip = function(settings_) {
		//---
